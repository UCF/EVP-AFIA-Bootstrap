...
class Person extends CustomPostType
{
	...
	public function get_objects($options=array()){
		$options['order']    = 'ASC';
		$options['orderby']  = 'profile_orderby_name';
		$options['meta_key'] = 'profile_orderby_name';
		return parent::get_objects($options);
	}

	public static function get_name($person) {
		$prefix = get_post_meta($person->ID, 'profile_title_prefix', True);
		$suffix = get_post_meta($person->ID, 'profile_title_suffix', True);
		$name = $person->post_title;
		return $prefix.' '.$name.' '.$suffix;
	}

	public static function get_phones($person) {
		$phones = get_post_meta($person->ID, 'profile_phones', True);
				...
				foreach($people as $person) {
					...
					$link = ($person->post_content == '') ? False : True; ?>
						<tr>
							...
							<td class="job_title">
								<?if($link) {?><a href="<?=get_permalink($person->ID)?>"><?}?>
								<?=get_post_meta($person->ID, 'profile_jobtitle', True)?>
								<?if($link) {?></a><?}?>
							</td>
